...
final class DownloadsAndSettingsTests: XCTestCase {
    ...
    func testAboutSectionExists() throws {
        ...
        let veydrune = app.staticTexts.matching(
            NSPredicate(format: "label CONTAINS[c] 'Veydrune' OR label CONTAINS[c] 'Version'")).firstMatch

        XCTAssertTrue(veydrune.waitForExistence(timeout: 5),
