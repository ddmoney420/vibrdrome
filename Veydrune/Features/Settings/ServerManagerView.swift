...
struct ServerEditView: View {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        ...
                        if case .edit(let server) = mode, password.isEmpty {
                            ...
                            finalPassword = KeychainAccess.Keychain(service: "com.veydrune")["server_\(server.id)"] ?? ""
                        } else {
    ...
    private func testConnection() {
        ...
        Task {
            ...
            if case .edit(let server) = mode, password.isEmpty {
                testPassword = KeychainAccess.Keychain(service: "com.veydrune")["server_\(server.id)"] ?? ""
            } else {
