...
final class PermissivePresetTests: XCTestCase {
    ...
    func testLibraryDecodesSixtySixPresets() {
        let presets = PermissivePresetLibrary.presets
        XCTAssertEqual(presets.count, 66)                     // 50 2D + 16 3D/procedural scenes
        XCTAssertEqual(presets.first?.name, "Flux")           // hero is index 0 (default on open)
        XCTAssertEqual(Set(presets.map(\.id)).count, 66)      // ids are unique
    ...
    func testThreeDScenesAreSixteenSceneModes() {
        ...
        // Exactly those sixteen are non-2D; everything else stays on the 2D engine (sceneMode 0).
