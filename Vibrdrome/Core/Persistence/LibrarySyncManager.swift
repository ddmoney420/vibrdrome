...
let syncLog = Logger(subsystem: "com.vibrdrome.app", category: "LibrarySync")
...
final class LibrarySyncManager {
    ...
    nonisolated static func hasAlbumChanged(_ cached: CachedAlbum, _ server: Album) -> Bool {
        ...
        cached.isCompilation != (server.isCompilation ?? false) ||
        cached.playCount != (server.playCount ?? 0) ||
        cached.releaseType != server.releaseTypes?.first ||
        cached.mood != server.moods?.first ||
        cached.sortAlbumName != server.sortName ||
            ...
            for artist in index.artist ?? [] {
                ...
                if let existing = localMap[artist.id] {
                    if hasArtistChanged(existing, artist) {
                        ...
                        existing.artistImageUrl = artist.artistImageUrl
                        existing.userRating = artist.userRating
                        existing.averageRating = artist.averageRating
    ...
    nonisolated static func hasArtistChanged(_ cached: CachedArtist, _ server: Artist) -> Bool {
        ...
        cached.isStarred != (server.starred != nil) ||
        cached.artistImageUrl != server.artistImageUrl ||
        cached.userRating != server.userRating ||
        cached.averageRating != server.averageRating
    }
    ...
    nonisolated static func updateSongFields(_ cached: CachedSong, from song: Song) {
        ...
        cached.channels = song.channelCount
        ...
        if let played = song.played { cached.lastPlayed = syncISO8601.date(from: played) }
        cached.playCount = Int(song.playCount ?? 0)
